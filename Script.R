...
points<-read.csv2("cities.csv") %>% select(City,lat,lon) #upload our file to R and select 3 columns
...
points(points$lon,points$lat, pch=19, cex = 0.5, col="Red" ) #add points ot our map
r@crs #verify our coordinate system
centroid_spdf <- SpatialPointsDataFrame(points[,2:3], proj4string=r@crs, points) #convert points to spatialdatafram
...
stations_data <- raster::extract(r_brick,             # raster layer
                                 ...
                                 #fun=na.rm,         # what to value to extract
...
colnames(toolik_df)<-c("Day", "Bogotá","Abu Dhabi","Moskva","Panama","Buenos Aires","Nairobi","Budapest","Jakarta","Quito","La paz","Paris","Berlin")
...
write.csv(data_long,"data_stations_daily_temperature_2m.csv") ##Export data in csv to your directory
